...
export const resolveMatch = (match: Match, gameData: GameData, format: Format): Match => {
    ...
    const getTeamFromStanding = (rank: number, groupName: string) => {
        ...
        const filtered = sortedStandings.filter(s => {
            ...
            if (groupName === 'Round-Robin') {
                return team?.group === 'Round-Robin' || team?.group === 'Super Sixes' || !team?.group || team?.group === 'Group A' || team?.group === 'Group B'; // Fallback to anyone if not set, but prioritize elite
            }
            ...
            return team?.initialGroup === groupKey || team?.group === groupKey;
        });
...
export const generateLeagueSchedule = (teams: Team[], format: Format, doubleRoundRobin: boolean = true): Match[] => {
    const matches: Match[] = [];
    if (teams.length < 16) return []; // Require full 16 teams for this specific format

    // Initial setup: strictly separate 6 elite and 10 qualifier teams
    // In a new game, we can use overall rating. 
    // In career mode, the promotion/relegation logic will have already set the 'group' property.
    const eliteTeams = teams.filter(t => t.group === 'Round-Robin' || t.group === 'Super Sixes').slice(0, 6);
    const qualifierTeams = teams.filter(t => t.group === 'A' || t.group === 'B' || !t.group).slice(0, 10);

    // If teams don't have groups yet (start of game), assign them
    const effectiveElite = eliteTeams.length === 6 ? eliteTeams : [...teams].sort((a,b) => (b.overallRating || 0) - (a.overallRating || 0)).slice(0, 6);
    const effectiveQualifiers = qualifierTeams.length === 10 ? qualifierTeams : teams.filter(t => !effectiveElite.find(et => et.id === t.id)).slice(0, 10);

    // 1. Qualifiers Stage (10 teams, 4 matches each)
    let matchCounter = 1;
    for (let round = 1; round <= 4; round++) {
        const shuffled = [...effectiveQualifiers].sort((a, b) => hashString(a.id + round) - hashString(b.id + round));
        for (let i = 0; i < shuffled.length; i += 2) {
            if (i + 1 < shuffled.length) {
                matches.push({
                    matchNumber: matchCounter++,
                    teamA: shuffled[i].name, teamAId: shuffled[i].id, vs: 'vs',
                    teamB: shuffled[i+1].name, teamBId: shuffled[i+1].id,
                    date: `Qualifier Round ${round}`, group: 'Group A' as any // Use A/B for different sets of qualifiers if needed, or just one group
                });
            }
        }
    }

    // 2. Main League Stage (6 teams, single or double round robin)
    for (let i = 0; i < effectiveElite.length; i++) {
        for (let j = i + 1; j < effectiveElite.length; j++) {
            matches.push({
                matchNumber: matchCounter++,
                teamA: effectiveElite[i].name, teamAId: effectiveElite[i].id, vs: 'vs',
                teamB: effectiveElite[j].name, teamBId: effectiveElite[j].id,
                date: `League Day ${i + j}`, group: 'Round-Robin' as any
            });
            if (doubleRoundRobin) {
                matches.push({
                    matchNumber: matchCounter++,
                    teamA: effectiveElite[j].name, teamAId: effectiveElite[j].id, vs: 'vs',
                    teamB: effectiveElite[i].name, teamBId: effectiveElite[i].id,
                    date: `League Day ${i + j} (R)`, group: 'Round-Robin' as any
                });
            }
        }
    }

    // 3. Knockouts (Top 4 from Round-Robin)
