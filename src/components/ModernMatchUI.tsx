...
interface ModernMatchUIProps {
    ...
    onSetBowlingStrategy: (s: Strategy) => void;
    onPlayBall: () => void;
    onPlayOver: () => void;
...
const BatsmanCard = ({ player, performance, isActive }: { player: Player, performance: BattingPerformance, isActive: boolean }) => {
    // Generate some stable aesthetic data for the area chart
    const chartData = useMemo(() => {
        const seed = player.name + (isActive ? 'active' : 'idle');
...
export const ModernMatchUI: React.FC<ModernMatchUIProps> = ({ state, gameData, onSetBattingStrategy, onSetBowlingStrategy, onPlayBall, onPlayOver, onShowMatchCentre }) => {
            ...
            <div className="flex-1 flex flex-col gap-4">
                ...
                <div className="mt-auto flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-neutral-900/60 ring-1 ring-white/10 rounded-[2rem] p-4 flex flex-col gap-3">
                             <div className="flex justify-between items-center">
                                <span className="text-[10px] text-white/30 font-black uppercase tracking-widest">BATTING AGRESSION</span>
                                <span className="text-[10px] text-teal-500 font-black uppercase tracking-widest">{state.strategies.batting.replace('_', ' ')}</span>
                             </div>
                             <div className="flex bg-black/40 rounded-full p-1 border border-white/5 overflow-x-auto scrollbar-hide">
                                {(['ultra_defensive', 'defensive', 'balanced', 'attacking', 'ultra_attacking'] as Strategy[]).map(s => (
                                    <button
                                        key={s}
                                        onClick={() => onSetBattingStrategy(s)}
                                        className={`px-3 py-2 text-[8px] uppercase font-black rounded-full transition-all whitespace-nowrap flex-1 ${state.strategies.batting === s 
                                            ? 'bg-teal-500 text-black shadow-lg' 
                                            : 'text-white/20 hover:text-white/40'}`}
                                    >
                                        {s === 'ultra_defensive' ? 'U-DEF' : s === 'ultra_attacking' ? 'U-ATT' : s.slice(0, 3)}
                                    </button>
                                ))}
                             </div>
                        </div>
                        ...
                        <div className="bg-neutral-900/60 ring-1 ring-white/10 rounded-[2rem] p-4 flex flex-col gap-3">
                             <div className="flex justify-between items-center">
                                <span className="text-[10px] text-white/30 font-black uppercase tracking-widest">BOWLING AGRESSION</span>
                                <span className="text-[10px] text-teal-400 font-black uppercase tracking-widest">{state.strategies.bowling.replace('_', ' ')}</span>
                             </div>
                             <div className="flex bg-black/40 rounded-full p-1 border border-white/5 overflow-x-auto scrollbar-hide">
                                {(['ultra_defensive', 'defensive', 'balanced', 'attacking', 'ultra_attacking'] as Strategy[]).map(s => (
                                    <button
                                        key={s}
                                        onClick={() => onSetBowlingStrategy(s)}
                                        className={`px-3 py-2 text-[8px] uppercase font-black rounded-full transition-all whitespace-nowrap flex-1 ${state.strategies.bowling === s 
                                            ? 'bg-teal-500 text-black shadow-lg' 
                                            : 'text-white/20 hover:text-white/40'}`}
                                    >
                                        {s === 'ultra_defensive' ? 'U-DEF' : s === 'ultra_attacking' ? 'U-ATT' : s.slice(0, 3)}
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        <motion.button 
                            whileTap={{ scale: 0.95 }}
                            onClick={onPlayBall}
                            className="bg-neutral-800/80 hover:bg-neutral-700/80 text-white border border-white/10 py-6 rounded-[2rem] flex flex-col items-center justify-center gap-1 group transition-all"
                        >
                            <Icons.Zap className="w-6 h-6 text-teal-500 group-hover:scale-125 transition-transform" />
                            <span className="font-black italic uppercase tracking-tighter text-lg">BALL</span>
                        </motion.button>
                        <motion.button 
                            whileTap={{ scale: 0.95 }}
                            onClick={onPlayOver}
                            className="bg-neutral-800/80 hover:bg-neutral-700/80 text-white border border-white/10 py-6 rounded-[2rem] flex flex-col items-center justify-center gap-1 group transition-all"
                        >
                            <Icons.FastForward className="w-6 h-6 text-yellow-500 group-hover:scale-125 transition-transform" />
                            <span className="font-black italic uppercase tracking-tighter text-lg">OVER</span>
                        </motion.button>
                        <motion.button 
                            whileTap={{ scale: 0.95 }}
                            ...
                            className="bg-neutral-800/80 hover:bg-neutral-700/80 text-white border border-white/10 py-6 rounded-[2rem] flex flex-col items-center justify-center gap-1 group transition-all"
                        >
                            <Icons.LayoutDashboard className="w-6 h-6 text-white/40 group-hover:scale-125 transition-transform" />
                            <span className="font-black italic uppercase tracking-tighter text-lg">CENTRE</span>
                        </motion.button>
                    </div>
                </div>
            </div>
